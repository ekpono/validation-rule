  ...
  async validation(req, res) {
    ...
    if (jsonObject) {
      return res
        ...
        .send({ message: jsonObject, status: "error", data: null });
    }
    ...
    if (rule.field.toString().indexOf(".") > -1) {
      ...
      if (
        ruleObj.length > 2 &&
        typeof secondLevelObject[ruleObj[1]] === "object"
      ) {
        return res
          .status(HttpStatus.BAD_REQUEST)
          .send({
      ...
      let nestedObjectFieldExist = objectValidation.validate(
        rule,
        data,
        ruleField,
        ruleObj
      );
      if (nestedObjectFieldExist)
        return res
          .status(HttpStatus.BAD_REQUEST)
          .send({
            message: nestedObjectFieldExist,
            status: "error",
            data: null,
          });
      ...
      if (ruleValidated) {
        return res
          ...
          .send(
            httpResponse.successResponse(rule.field, rule.condition_value, rule)
          );
      }
      ...
      return res
        .status(HttpStatus.BAD_REQUEST)
        .send(
          httpResponse.invalidationFailedResponse(
            rule.field,
    ...
    if (isNumberGiven) {
      ...
      if (typeof data === "string") {
        ...
        if (ruleValidated) {
          return res
            .status(HttpStatus.OK)
            .send(httpResponse.successResponse(rule.field, value, rule));
        }
        return res
          .status(HttpStatus.BAD_REQUEST)
          .send(
            httpResponse.invalidationFailedResponse(rule.field, value, rule)
          );
      }
      ...
      if (typeof data === "object") {
        ...
        if (ruleValidation) {
          return res
            .status(HttpStatus.OK)
            .send(
              httpResponse.successResponse(rule.field, condition_value, rule)
            );
        }
        return res
          .status(HttpStatus.BAD_REQUEST)
    ...
    if (rule.field in data) {
      ...
      return res
        .status(HttpStatus.BAD_REQUEST)
        .send(httpResponse.invalidationFailedResponse(rule.field, value, rule));
    }
  },
};
